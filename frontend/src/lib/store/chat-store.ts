...
export const useChatStore = create<FullChatStore>()(
    immer((set, get) => ({
         ...
         */
        _addOrUpdateConversation: (sessionId: string, title = 'New conversation') =>
            set((state) => {
                            ...
                            if (response && response.session_id) {
                                ...
                                if (responseSessionId !== currentSessionId) {
                                    ...
                                    get()._addOrUpdateConversation(
                                        responseSessionId,
                                        response.title || message.substring(0, 40),
                                    );
        ...
        deleteConversationById: async (sid: string) => {
            ...
            const isLocalOnly = state.conversations.some(
                (c) => c.id === sid && (!c.createdAt || c.createdAt.startsWith('local_')),
            );
