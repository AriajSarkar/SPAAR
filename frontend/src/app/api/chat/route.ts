...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!body || !body.prompt) {
            return NextResponse.json({ error: 'Prompt parameter is required in the request body' }, { status: 400 });
        }
            ...
            const readableStream = new ReadableStream({
                async start(controller) {
                    try {
                        ...
                        await generateLLMResponse(prompt, session_id, include_history, (chunk) => {
